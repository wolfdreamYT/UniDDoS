...
def send_requests(target, max_requests, delay, thread_id):
    ...
    while i < max_requests and not stop_flag:
        ...
        proxies = None
        if proxies_list:
            proxy_choice = random.choice(proxies_list)
            proxies = {"http": proxy_choice, "https": proxy_choice}

        try:
            response = requests.get(target, headers=headers, proxies=proxies, timeout=5)
            status = 1 if response.status_code == 200 else 0
        except:
            status = 0

        data_points.append(status)
        print(f"[Thread-{thread_id}] [{i+1}] Status: {status}, UA: {headers['User-Agent']}, IP: {headers['X-Forwarded-For']}")
        i += 1
        time.sleep(delay)
...
ttk.Label(frame, text="Target URL/IP:").grid(row=0, column=0, sticky="w")
entry_target = ttk.Entry(frame, width=30)
entry_target.grid(row=0, column=1)
entry_target.insert(0, "http://localhost:8000")

ttk.Label(frame, text="Requests:").grid(row=1, column=0, sticky="w")
entry_requests = ttk.Entry(frame, width=30)
entry_requests.grid(row=1, column=1)
entry_requests.insert(0, "1000")
